...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="sitk-dicomseries",
        description="Scan directory trees for DICOM series and optionally convert them."
    )
    ...
    parser.add_argument("-v", "--verbose", action="count", default=0,
                        help="Increase verbosity level (use -vv for debug output).")
    parser.add_argument("-r", "--recursive", action="store_true",
                        help="Search directories recursively.")
    parser.add_argument("-c", "--convert", action="store_true",
                        help="Convert each series to a volume.")
    parser.add_argument("-d", "--dict", action="store_true",
                        help="Print the metadata dictionary.")
    parser.add_argument("-s", "--suffix", default=".nrrd", metavar="string",
                        help="Output volume file suffix (default: .nrrd).")
    parser.add_argument("-t", "--thickness", type=int, default=20, metavar="int",
                        help="Minimum Z thickness in slices for conversion (default: 20).")
    parser.add_argument(
        "-n", "--name", dest="name_src", default="desc",
        choices=["desc", "series"],
        help="Output naming source: 'desc' (series description) or 'series' (series ID).",
    )
